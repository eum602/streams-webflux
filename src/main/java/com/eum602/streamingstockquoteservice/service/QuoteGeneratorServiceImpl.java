...
public class QuoteGeneratorServiceImpl implements QuoteGeneratorService {
    ...
    @Override //this method is going to generate an endless stream
    ...
    private Quote updateQuote(Quote quote) {
        BigDecimal priceChange = quote.getPrice()
                .multiply(new BigDecimal(0.05 * this.random.nextDouble()), this.mathContext);//generate a new random price
        return new Quote(quote.getTicker(), quote.getPrice().add(priceChange)); //as part of immutabilitity returns back a new quote object
    }
}
